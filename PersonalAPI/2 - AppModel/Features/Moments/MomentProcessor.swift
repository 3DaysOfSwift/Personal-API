...
actor LocalJournalFactExtractor: JournalFactExtracting {
    ...
    func extract(from moment: MomentSnapshot) throws -> [PersonalFactSnapshot] {
        let topics: [(String, String)] = [
            ("Occupation work career", #"\b(my (job|occupation|profession|career)|i (work|worked|am employed|was employed))\b"#),
            ("Preferences enjoyment interests", #"\bi (love|like|enjoy|hate|dislike|prefer|loved|liked|enjoyed|hated|disliked|used to)\b"#),
        ...
        for paragraph in moment.text.components(separatedBy: .newlines) {
            ...
            let labels = topics.compactMap { label, pattern in
                passage.range(of: pattern, options: [.regularExpression, .caseInsensitive]) == nil ? nil : label
            }
            guard !labels.isEmpty else { continue }
            facts.append(PersonalFactSnapshot(id: UUID(), label: labels.joined(separator: "; "),
                value: passage, createdAt: now(), derivation: FactDerivation(momentID: moment.id,

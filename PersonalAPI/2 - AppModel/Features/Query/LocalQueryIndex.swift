...
actor LocalQueryIndex {
  ...
  private let vocabulary: [Set<String>] = [
    ...
    [
      "hobby", "hobbies", "interest", "interests", "leisure", "gardening", "painting",
      "photography", "music",
    ],
  ...
  func select(
    question: String, previousQuestions: [String], moments: [MomentSnapshot],
    facts: [PersonalFactSnapshot] = []
  ) throws -> Selection {
    ...
    let references: Set<String> = [
      "it", "he", "she", "him", "her", "they", "them", "there", "then", "that", "those",
    ]
    let needsContext = !tokens(question).isDisjoint(with: references)
    let history = needsContext ? Array(previousQuestions.suffix(4)) : []
    let historyTerms = history.reduce(into: Set<String>()) {
      $0.formUnion(tokens($1).subtracting(stop))
    }
    ...
    let broad =
      question.lowercased().contains("throughout") || question.lowercased().contains("over my life")
      || question.lowercased().contains("over time")
      || question.lowercased().contains("across my life")
    ...
    for moment in moments {
      ...
      for fact in supported {
        ...
        guard direct + contextual > 0,
          focusTerms.isEmpty || !terms.isDisjoint(with: focusTerms)
        else { continue }
    ...
    for candidate in ranked {
      guard
        !selected.contains(where: {
          $0.moment.id == candidate.moment.id && $0.text == candidate.text
        })
      else { continue }
    ...
    let snapshots = selected.map { candidate in
      MomentSnapshot(
        id: candidate.moment.id, text: candidate.text,
        createdAt: candidate.moment.createdAt, happenedAt: candidate.moment.happenedAt,
        source: candidate.moment.source, analysisData: nil,
        processingState: candidate.moment.processingState)
    }
    return Selection(
      candidates: snapshots, previousQuestions: history, indexedPassageCount: sequence)
  }
}

...
enum LifeMapError: LocalizedError {
    ...
    var errorDescription: String? {
        switch self {
        case .unavailable: "Life Map needs on-device Apple Intelligence. Enable it on a supported device, then try again."
        case .refused: "Apple Intelligence couldn’t process some entries under its content restrictions. Your original entries are unchanged. Other entries can still appear on your map."
        case .invalidResponse: "Apple Intelligence couldn’t produce a usable result for some entries. You can try those entries again."
        case .invalidEvidence: "The AI returned a Moment without a matching journal passage. That entry needs another attempt."
        }
    }
}
actor LifeMapExtractor: LifeMapExtracting {
    func extract(_ text: String) async throws -> [LifeMapCandidate] {
        ...
        if #available(iOS 26.0, macOS 26.0, *) {
            ...
            let event = DynamicGenerationSchema(name: "JournalEvent", properties: [
                .init(name: "title", description: "A specific label of at most six words", schema: .init(type: String.self)),
                .init(name: "passage", description: "An exact contiguous quotation from the journal", schema: .init(type: String.self))
            ])
            let schema = try GenerationSchema(root: .init(name: "JournalEvents", properties: [
            ...
            while !remaining.isEmpty {
                ...
                let end = remaining.index(remaining.startIndex, offsetBy: 2400, limitedBy: remaining.endIndex) ?? remaining.endIndex
                ...
                """)
                do {
                    let response = try await session.respond(to: "Journal passage:\n\(chunk)", schema: schema)
                    let events = try response.content.value([GeneratedContent].self, forProperty: "events")
                    result.append(contentsOf: try events.map {
                        LifeMapCandidate(title: try $0.value(String.self, forProperty: "title"),

...
@MainActor @Observable final class SettingsViewModel {
    ...
    func prepareExport() async -> Bool {
        ...
        do { filename = ExportDocument.datasetFilename(); document = ExportDocument(data: try await settings.exportDataset()); message = nil; return true }
        catch { document = nil; message = error.localizedDescription; return false }
    }
    ...
    func regenerate() async {
        ...
        isWorking = true; defer { isWorking = false }
        do { try await settings.regenerateMetadata(); message = settings.enrichmentError ?? "Derived metadata regenerated." }

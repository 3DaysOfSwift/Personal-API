...
@MainActor final class QueryManagerTests: XCTestCase {
    func testLegacyFactsArePreservedButExcludedFromJournalSearch() async throws {
        ...
        let fact = PersonalFactSnapshot(id: UUID(), label: "Occupation", value: "iOS developer", createdAt: Date())
        try await repository.saveFact(fact)
        let manager = QueryManager(repository: repository, retriever: MomentRetriever(), index: LocalQueryIndex(),
    ...
    func testChatContextIsBoundedAndRetryReadsNewJournalEvidence() async throws {
        let repository = MemoryRepository()
        let search = ContextSearchCapture()
        let query = QueryManager(repository: repository, retriever: MomentRetriever(), index: LocalQueryIndex(), semanticSearch: search, answerer: SourceAnswer())
    ...
    func testRelevantTailReachesAnswerInsteadOfUnrelatedEntryBeginning() async throws {
        ...
        let manager = QueryManager(repository: repository, retriever: MomentRetriever(), index: LocalQueryIndex(), semanticSearch: search, answerer: ContextAnswer())
    ...
    func testSearchFailureDoesNotDisableNextQuestion() async throws {
        ...
        let manager = QueryManager(repository: repository, retriever: MomentRetriever(), index: LocalQueryIndex(), semanticSearch: RecoveringSearch(), answerer: ContextAnswer())
    ...
    func testAnswerFailureDoesNotDisableNextQuestion() async throws {
        let source = moment("I attended school.")
        let repository = MemoryRepository(); try await repository.saveMoment(source)
        let manager = QueryManager(repository: repository, retriever: MomentRetriever(), index: LocalQueryIndex(), semanticSearch: SearchStub(ids: [source.id]), answerer: RecoveringAnswer())
        let failed = try await manager.search("school")
    ...
    func testJobQuestionNeverSendsSchoolOnlyEntryToAI() async throws {
        ...
        try await repository.saveMoment(school); try await repository.saveMoment(job)
        let capture = ContextSearchCapture()
        let query = QueryManager(repository: repository, retriever: MomentRetriever(), index: LocalQueryIndex(), semanticSearch: capture, answerer: ContextAnswer())
        let result = try await query.search("What is my job?", previousQuestions: ["Did I enjoy primary school?"])
    ...
    func testChildhoodRemainsSearchableAndNewMemoriesUpdateIndex() async throws {
        ...
        let first = try await index.select(question: "Did I attend school?", previousQuestions: [], moments: [school, job])
        XCTAssertEqual(Set(first.candidates.map(\.id)), [school.id])
        let added = moment("I liked my classroom lessons.")
        let refreshed = try await index.select(question: "Did I enjoy school?", previousQuestions: [], moments: [school, job, added])
    ...
    func testNoCandidatesMakesNoAIRequest() async throws {
        ...
        let capture = ContextSearchCapture()
        let query = QueryManager(repository: repository, retriever: MomentRetriever(), index: LocalQueryIndex(), semanticSearch: capture)
    ...
    func testFollowUpRetainsSubjectButWorkTopicChangeDropsIt() async throws {
        ...
        let selected = try await index.select(question: "Where did I meet him?", previousQuestions: ["Who was Alex?"], moments: [school])
    ...
    func testCandidateSelectionIsBoundedAndKeepsExactSourceText() async throws {
        ...
        let selected = try await LocalQueryIndex().select(question: "How have my hobbies changed throughout my life?", previousQuestions: [], moments: moments)
        XCTAssertEqual(selected.candidates.count, 12)
        XCTAssertTrue(selected.candidates.allSatisfy { candidate in
    ...
    func testDirectLiveJobAnswersWhenRequested() async throws {
        ...
        let source = moment("My job is an iOS developer. I love making iOS apps. I am proud to be an iOS developer.")
        for question in ["What is my job?", "Do I like my job?", "Do I make apps?"] {
            let answer = try await OnDeviceMomentAnswerer().answer(question: question, evidence: [Evidence(moment: source, score: 1)])
            let text = try XCTUnwrap(answer?.text, "Model abstained despite explicit evidence: \(question)").lowercased()
    ...
    func testLiveJobPipelineWhenRequested() async throws {
        ...
        let repository = MemoryRepository()
        try await repository.saveMoment(moment("My job is an iOS developer. I love making iOS apps. I am proud to be an iOS developer."))
        let manager = QueryManager(repository: repository, retriever: MomentRetriever(), index: LocalQueryIndex(),
            semanticSearch: OnDeviceMomentSearch(), answerer: OnDeviceMomentAnswerer())
        for question in ["What is my job?", "Do I like my job?", "Do I make apps?"] {
            let result = try await manager.search(question)
            XCTAssertFalse(result.evidence.isEmpty, "No evidence selected for \(question); stage: \(String(describing: result.failureStage))")
            XCTAssertNotNil(result.generatedAnswer, "No answer for \(question); stage: \(String(describing: result.failureStage)); issue: \(result.answerIssue ?? "none"); missing evidence: \(result.needsMoreMemories)")
    ...
    """
        ...
        add(attachment)
        #endif
        XCTAssertEqual(serviceErrors, 0, "See Prompt and data comparison attachment for per-request errors.")
    ...
    private func assertJobAnswer(_ text: String, question: String, file: StaticString = #filePath, line: UInt = #line) {
        let text = text.lowercased()
        XCTAssertFalse(text.contains("cannot answer") || text.contains("not enough") || text.contains("insufficient"), text, file: file, line: line)
    ...
    func testConversationalAnswerDoesNotRequireGeneratedQuotations() async throws {
        ...
        let manager = QueryManager(repository: repository, retriever: MomentRetriever(), index: LocalQueryIndex(), semanticSearch: SearchStub(ids: [source.id]), answerer: AnswerStub(result: answer))
    ...
    func testAnswerAbstentionRetainsSourcesAndExplainsInsufficientEvidence() async throws {
        ...
        let repository = MemoryRepository(); try await repository.saveMoment(source)
        let manager = QueryManager(repository: repository, retriever: MomentRetriever(), index: LocalQueryIndex(), semanticSearch: SearchStub(ids: [source.id]), answerer: AnswerStub(result: nil))
    ...
    func testSemanticMatchDoesNotRequireKeywordOverlapAndPreservesOriginal() async throws {
        ...
        try await repository.saveMoment(original)
        let manager = QueryManager(repository: repository, retriever: MomentRetriever(), index: LocalQueryIndex(), semanticSearch: SearchStub(ids: [original.id, original.id]))
    ...
    func testUnavailableModelReturnsClearlyLabelledKeywordFallback() async throws {
        ...
        try await repository.saveMoment(source)
        let manager = QueryManager(repository: repository, retriever: MomentRetriever(), index: LocalQueryIndex(), semanticSearch: SearchStub(ids: [], failure: .unavailable("Model not ready")))
    ...
    func testInventedSourceIDCannotBecomeEvidence() async throws {
        ...
        let manager = QueryManager(repository: repository, retriever: MomentRetriever(), index: LocalQueryIndex(), semanticSearch: SearchStub(ids: [UUID()]))
        let result = try await manager.search("exercise")
        XCTAssertTrue(result.evidence.isEmpty)
    ...
    func testCancellationDoesNotStartFallback() async throws {
        ...
        let manager = QueryManager(repository: repository, retriever: MomentRetriever(), index: LocalQueryIndex(), semanticSearch: SearchStub(ids: [], cancel: true))
    ...
    func testAIAbstentionStaysEmptyEvenWithKeywordOverlap() async throws {
        ...
        let manager = QueryManager(repository: repository, retriever: MomentRetriever(), index: LocalQueryIndex(), semanticSearch: SearchStub(ids: []))
        let result = try await manager.search("garden")
        XCTAssertEqual(result.method, .onDeviceAI)
        XCTAssertTrue(result.evidence.isEmpty)
        XCTAssertTrue(result.needsMoreMemories)
...
extension QueryManagerTests {
    func testDerivedLabelsFindOriginalPassageAndRejectStaleOrOrphanedFacts() async throws {
        ...
        let index = LocalQueryIndex()
        let selected = try await index.select(question: "What possessions?", previousQuestions: [], moments: [source], facts: facts)
        XCTAssertEqual(selected.candidates.first?.text, source.text)
        XCTAssertEqual(selected.candidates.first?.id, source.id)
        let absent = try await index.select(question: "What possessions?", previousQuestions: [], moments: [], facts: facts)
        XCTAssertTrue(absent.candidates.isEmpty)
        let edited = MomentSnapshot(id: source.id, text: "The weather was sunny.", createdAt: source.createdAt,
            happenedAt: nil, source: "manual", analysisData: nil, processingState: "pending")
        let stale = try await index.select(question: "What possessions?", previousQuestions: [], moments: [edited], facts: facts)

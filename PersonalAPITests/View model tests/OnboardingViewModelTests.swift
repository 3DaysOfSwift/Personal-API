...
@MainActor final class OnboardingViewModelTests: XCTestCase {
    func testCompletingOnboardingPersistsAcrossManagerReconstruction() {
        ...
        vm.startDataset()
        let reloaded = SettingsManager(preferences: graph.preferences, repository: graph.repository, moments: graph.app.momentsFeature, profile: graph.app.profileFeature)
